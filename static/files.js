...
const parseEntryObjects = () => {
  document.querySelectorAll('.entry[data-extra]').forEach(el => {
    ...
    for (const attr in obj) {
      ...
      // A list (the other names) is kept one item on each line. A name can have a comma.
...
function __scoreByName(el, query) {
  ...
  // The romaji, pinyin, Chinese or Japanese titles, one on each line.
...
function filterEntries(query) {
  ...
  // A Bangumi URL names one show. A bare number is a name, not a subject.
  let bangumiId = query.includes('/subject/') ? getBangumiId(query) : null;
  let mapped = [];
  if (asin !== null) {
  ...
  } else if (bangumiId !== null) {
    mapped = entries.map(e => {
      return {
        entry: e,
        score: parseInt(e.dataset.bangumiId, 10) === bangumiId ? 0 : MIN_SCORE,
      };
    });
